...
for (k in 1:length(n)) {
  ...
  for (i in 1:R) {
    ...
    mat1[i, , k] <- quantile(boot_fsn, c(0.025, 0.975), na.rm = TRUE)
  }
...
for (k in 1:length(n)) {
  ...
  for (i in 1:R) {
    ...
    # Generate half-normal observations
    z <- abs(rnorm(n[k]))
    S <- sum(z)
    ...
    mat2[i, , k] <- quantile(boot_fsn, c(0.025, 0.975), na.rm = TRUE)
  }
...
for (k in 1:length(n)) {
  ...
  for (i in 1:R) {
    ...
    # Bootstrap distribution of the FSN 
    boot_fsn <- numeric(B)
    ...
    mat4[i, , k] <- quantile(boot_fsn, c(0.025, 0.975), na.rm = TRUE)
...
for (k in 1:length(n)) {
  ...
  for (i in 1:R) {
    ...
    mat5[i, , k] <- quantile(boot_fsn, c(0.025, 0.975), na.rm = TRUE)
